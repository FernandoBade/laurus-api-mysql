...
export async function createLog(
    ...
    const logMessage = typeof detail === "object" && Object.keys(detail).length === 0
        ? "Unknown error"
...
/**
 * Formats validation errors from Zod into a standardized format.
 *
    ...
    return error.errors.map((e: ZodIssue) => {
        ...
        switch (e.code) {
            case "invalid_enum_value":
                ...
                break;
            case "invalid_type":
                translatedMessage = `Field '${e.path.join('.')}' expects a '${(e as any).expected}' type but received '${(e as any).received}'`;
                break;
            case "too_small":
            ...
            case "unrecognized_keys":
                translatedMessage = `Field '${(e as any).keys.join(", ")}' is unrecognized and should not be included in this request.`;
                break;
            case "invalid_date":
...
export async function getById(table: TableName, id: number) {
    ...
    return result.length ? result[0] : { error: `${table} not found` };
}
...
/**
 * Searches for records in a table based on an optional column filter.
 * @param table - The name of the table.
