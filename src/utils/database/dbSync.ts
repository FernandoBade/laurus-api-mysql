...
async function removeForeignKeyIfExists(tableName: string, columnName: string) {
    try {
        const [existingConstraints]: any[] = await db.query(`
        ...
        if (existingConstraints.length > 0) {
            ...
            createLog(LogType.DEBUG, LogOperation.UPDATE, LogCategory.DATABASE, `Dropping foreign key '${constraintName}' from '${tableName}'`);
            await db.query(`ALTER TABLE ${tableName} DROP FOREIGN KEY ${constraintName}`);
        }
    } catch (error) {
        createLog(LogType.ERROR, LogOperation.UPDATE, LogCategory.DATABASE, { message: `Failed to remove foreign key for '${tableName}.${columnName}'`, error });
    }
}
